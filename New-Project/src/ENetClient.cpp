...
std::string get_md5_checksum(const std::string& str) noexcept {
	const EVP_MD* md = EVP_md5();
	std::uint8_t md_value[EVP_MAX_MD_SIZE];
	std::uint32_t md_len;

	EVP_Digest(str.c_str(), str.size(), md_value, &md_len, md, NULL);

	std::ostringstream ss;
	ss << std::uppercase << std::hex << std::setfill('0') << std::setw(2);

	for (std::uint32_t i = 0; i < md_len; ++i)
		ss << static_cast<std::uint32_t>(md_value[i]);

	return ss.str();
}

std::string create_klv(float game_version, std::uint32_t protocol, std::int32_t hash, const std::string& rid) noexcept {
	...
	return get_md5_checksum(std::format(
		"{}{}{}{}{}{}{}{}", salts[0], game_version, salts[1], hash, salts[2], rid, salts[3], protocol
	));
}
...
std::string ClientData::Create() {
	...
	m_ret << "rid|" << "01D03A6A18E320A90AFB54C3147BE0B4" << "\n";
	m_ret << "platformID|" << "0,1,1" << "\n";
	m_ret << "deviceVersion|" << (int)0 << "\n";
	m_ret << "country|" << "it" << "\n";
	m_ret << "hash|" << (int)389977099 << "\n";
	m_ret << "mac|" << mac << "\n";
...
void ENetClient::OnTankPacket(int type, uint8_t* ptr, int size) {
	...
	case 1: {
		...
		if (func.find("OnSuperMain") != std::string::npos) {
			...
			std::string urls = "http://54.36.0.86/captcha=";
			http::Request request{ std::string{urls}.c_str() };
			const auto response = request.send("GET");
			std::string captchaAnswer = std::string{ response.body.begin(), response.body.end() };
		    SendPacket(2, "action|dialog_return\ndialog_name|puzzle_captcha_submit\ncaptcha_answer|" + captchaAnswer + "|CaptchaID|" + split[4]);
		}
